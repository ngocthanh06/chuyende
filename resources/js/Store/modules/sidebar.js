...
export default {
    ...
    getters: {
        ...
        getEmployerComp(state) { 
            return state.EmployerComp 
        },

        getDatecaLam(state) { 
            return state.DateCaLam 
        },
    ...
    actions: {
        allEmployers(context, [pagiSize, currentPage]) {
            ...
            axios.get(uri).then((res) => { 
                context.commit('commitEmployers', res.data) 
            });
        },
        ...
        allCompany(context) {
            axios.get('/api/company').then((res) => { 
                context.commit('commitCompany', res.data) 
            })
        },
        ...
        allEmployerComp(context, id) {
            axios.get(`api/empCompany/${id}`).then((res) => { 
                context.commit('commitEmpComp', res.data) 
            });
        },

        // Todo Lấy tất cả ca làm
        allCaLam(context) {
            axios.get('/api/CaLam').then((res) => { 
                context.commit('commitCaLam', res.data) 
            })
        },

        allRole(context) {
            axios.get(`/api/Role`).then((res) => { 
                context.commit('commitRole', res.data) 
            });
        },
    },
    ...
    mutations: {
        commitEmployers(state, data) { 
            state.district = data;
        },

        commitNumWeek(state, data) { 
            state.numWeek = data;
        },

        commitCompany(state, data) { 
            state.company = data;
        },

        commitEmpComp(state, data) { 
            state.EmployerComp = data; 
        },

        commitDateCalam(state, data) { 
            state.DateCaLam = data; 
        },

        commitCaLam(state, data) { 
            state.CaLam = data; 
        },

        commitRole(state, data) { 
            state.Role = data; 
        },
    },
}