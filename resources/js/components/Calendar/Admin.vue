                                ...
                                <div class="ctLists">
                                    <div v-for="(item, index) in getEmpCom()" :key="index" class="L_item">
                                        ...
                                        <div v-for="val in ChangeCaLam()" :key="val" class="i_desc titleTable">
                                            ...
                                            <div v-for="(value, index) in ValueCaLam" :key ="index" >
                                                    <div v-if="val === value.WS_date && item.User_id === value.User_id">
                                                        {{  value.FormM_name  }}
                                                </div>
                                            </div>
...
export default {
    ...
    methods: {
        ...
        setVal(){
            let count = 0;
            let num=[] ;
            axios.post('/api/countCaLam', {id : this.company}).then(res=>{
                res.data.forEach(
                    (val, index) => {
                        val.forEach(
                           (va,dex) =>{
...
<style lang="scss">
    #calendar{
        .card-body{
                flex: 1 1 auto; padding: 1.92rem 2.3rem;
            .list-view{
                height: 100%; display: flex; flex-direction: column;
                .title{
                    display: flex; margin-bottom: 10px; -ms-flex-pack: justify!important; margin-left: -0.5rem; 
                    justify-content: space-between!important;
                    .ttLeff{
                        .form-group{
                                float: left; margin: 0 0.5rem 0.5rem 0.5rem;
                            }
                        .flatpickr{
                            position: relative; height: 28px; cursor: pointer;
                            #datepicker{
                                background: #ffffff !important; height: 100%;
                            }
                        }
                    }
                    .ttRight{
                ...
                // end title
                .content{
                    .ctTitleSearch, .L_item{
                        height: 50px; overflow: hidden; display: flex;
                        .titleTableSearch{
                            flex: 0 1 200px;border: 1px solid #e3e3e3; margin: 0!important;
                            display: flex; justify-content: flex-start; align-items: center; height: 100%;
                            input{
                                border: none; outline: none
                            }
                        }
                        .titleTable{flex: 1 1 10px; border-width: 1px 1px 1px 0;border-style: solid;
                            border-color: #e3e3e3; margin: 0!important; display: flex;
                            justify-content: center; align-items: center; height: 100%; background-color: #f6f6f6;
                        }
                    }
                    .ctLists{
                        .i_desc, .i_name{
                            background-color:#ffffff; border-top: none; padding: 0 10px;
                            button{
                                color: rgba(0,0,0,.65); background-color: #fff; border-color: #e4e9f0;
                                border-style: dashed; align-self: center; border-width: 2px; border-color: #e3e3e3;
                                color: #e3e3e3; padding: 0; display: flex; justify-content: center;
                                align-content: center; width: 50%;
                            }
                            .logo{
                                    margin-right: 5px; width: 24px; height: 24px; line-height: 24px; 
                                    border-radius: 50%;
                                    img{
    ...
    .flatpickr-monthDropdown-months {
        background: transparent; border: none; border-radius: 0; box-sizing: border-box; color: inherit;
        cursor: pointer; font-size: inherit; font-family: inherit; font-weight: 900; height: auto;
        line-height: inherit; margin: -1px 0 0 0; outline: none; padding: 0 0 0 0.5ch;
        position: relative; vertical-align: initial; -webkit-box-sizing: border-box;
        -webkit-appearance: menulist; -moz-appearance: menulist;  width: auto;
        }
    .flatpickr-weekdaycontainer , #exampleFormControlSelect1{
        display: flex; height: 100%; width: 100%; align-items: center; margin-top: 10px;
        span{ color: #000 !important; }
    }
    #exampleFormControlSelect1{margin: 0; padding: 0 10px}
    .flatpickr-weekwrapper .flatpickr-weekday{position:relative; top: 5px; color: #000 !important;}
    .flatpickr-calendar{
