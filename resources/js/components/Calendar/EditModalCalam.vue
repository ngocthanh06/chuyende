  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="myModal">Ca làm đã đăng ký ngày <span style="color: red ">{{date}}</span></h5>
          ...
          <div v-for="(val, item) in Calam" :key="item" class="col-6 modal-shift-wrapper d-flex">
            <div class="cell-content large" style="border-color: rgba(102, 102, 102, 0.467); background-color: rgba(102, 102, 102, 0.133);">
              ...
              <div class="overlay-action">
                <button v-if="checkRole != 1"  type="button" class="action" trigger="click" @click="diemdanh(val.form_id, val.work_id, val.status)" href="#diemdanh" data-target="#diemdanh" data-toggle="modal" >
                ...
                <el-dropdown v-if="val.status == 0"  @command="delCaLam" style="width: 33%">
                  ...
                  <button style="display: none" ref="btns" href="#createPhep" data-toggle="modal" data-target="#createPhep" ></button>
                  <el-dropdown-menu slot="dropdown"> 
                    <el-button type="button" style="border: none" @click="createPhep(val)" icon="el-icon-edit-outline">Tạo yêu cầu nghỉ phép</el-button>
...
import moment from "moment";
 export default {
   ...
   methods: {
     ...
     delCaLam(command) {
       if (command.text === 'del') {
         this.$confirm('Bạn muốn hủy ca làm ? Tiếp tục?', 'Thông báo', {
             ...
             axios.post('/api/delCalam/', {
               work_id: command.value
             }).then(res => {
               if (res.status === 200) {
                 this.HandelPage()
               }
     ...
     * TODO input
     * @param val => att_id
     * ! $refs.btns[0] => call button open modal
     */
     createPhep(val){ 
       this.$refs.btns[0].click();
       this.$emit('createPheps',val);
     }
   },
   computed: {
       date(){
       ...
       checkRole(){
         return JSON.parse(localStorage.getItem('user')).role_id;
       }
   }
 }
</script>
...
<style lang="scss">
 .ant-modal-body {
   ...
   .timekeeping-list-date-cell-container {
     flex: 1;
     display: flex;
     white-space: normal;
     overflow: hidden;
     ...
     .modal-shift-wrapper {
       ...
       .cell-content:hover .overlay-action {
         visibility: visible;
       }
     }
