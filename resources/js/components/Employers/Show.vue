        ...
        <div class="card mb-grid">
          <div class="table-responsive-md">
            ...
            >
              <el-table
                v-loading="loading"
                :data="tableData.filter(data => !search || data.User_fullname.toLowerCase().includes(search.toLowerCase()))"
                ...
                <el-table-column label="Tùy chọn" align="right">
                  ...
                  <template slot-scope="scope">
                    <router-link
                      ...
                      :to="`/edit-Employer/${scope.row.User_id}`"
                    >Sửa</router-link>
                    <el-button
                      size="mini"
                      type="danger"
                      @click="handleDelete(scope.row.User_id, scope.row)"
                    >Khóa</el-button>
                  </template>
                </el-table-column>
              </el-table>
          ...
          axios.get(`/api/deleteEmployer/${event}`).then(res => {
            if (res.status === 200) {
              this.$message({
                type: "success",
                message:
                  "khóa thành công, vào danh sách nhân viên khóa để xem nhân viên vừa khóa"
              });
              this.getEmployer();
            }
          });
